...
class Solution {
	...
	public String[] solution(String[] orders, int[] course) {
		...
		for (int i = 0; i < orders.length; i++) {
			...
			Arrays.sort(arr); orders[i] = String.valueOf(arr);
			}
	...
	public void combination(String order, String others, int count) {
		// 탈출 조건 : count == 0 
		if (count == 0) {
			hashMap.put(order, hashMap.getOrDefault(order, 0) + 1); return;
			} 
		// 0부터 length까지 조합 
		for (int i = 0; i < others.length(); i++) combination(order + others.charAt(i), others.substring(i + 1), count - 1);
		} 
